...
static bool rkx_binder_buffer_data_equal(struct binder_proc* proc,
...
{
	u8 hdr[RKX_INTERFACETOKEN_BUFF_SIZE];
	size_t copy_size;
	size_t i = 0;
	size_t j;
	char* p;
	...
	rpc_name[0] = '\0';
	if (buffer->data_size <= RKX_PARCEL_OFFSET)
		return false;
	...
	struct binder_transaction* t2, u8 strategy)
{
	...
	if (t1->to_proc->tsk == t2->to_proc->tsk && t1->code == t2->code &&
		...
		if (strategy == RKX_FREE_ASYNC_BY_DATA)
			return rkx_binder_buffer_data_equal(t1->to_proc, t1->buffer, t2->buffer);
	}
	return false;
}
...
static inline void rk_binder_stats_deleted(enum binder_stat_types type)
{
...
static int __nocfi rkx_binder_proc_transaction_pre(struct kprobe* p, struct pt_regs* regs)
{
...
static struct kprobe kp_binder_proc_transaction = {
	...
	.pre_handler = rkx_binder_proc_transaction_pre
};

static bool rkx_kp_registered;
