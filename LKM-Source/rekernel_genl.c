...
#ifdef DEBUG
	pr_info("Re-Kernel addMonitorUid uid=%d\n", muid);
#endif
	net_uid_add(muid);
	return 0;
}
...
static const struct nla_policy rekernel_genl_policy[REKERNEL_A_MAX + 1] = {
	[REKERNEL_A_EVENT]  = { .type = NLA_NESTED },
 ...
 *     ├─ REKERNEL_A_EVENT_TYPE (u8)
 *     └─ REKERNEL_A_BINDER/_SIGNAL/_NETWORK (NLA_NESTED)
...
int sendMessage(struct rekernel_event *event)
{
    ...
    msg_head = genlmsg_put(skb, 0, 0, &rekernel_genl_family, 0, REKERNEL_C_EVENT);
    if (!msg_head) {
    ...
    /* REKERNEL_A_EVENT (outer nest) */
    evt = nla_nest_start(skb, REKERNEL_A_EVENT);
    if (!evt)
        goto nla_fail;

    if (nla_put_u8(skb, REKERNEL_A_EVENT_TYPE, event->type))
        goto nla_fail;

    switch (event->type) {
    case REKERNEL_EVT_BINDER: {
        struct rekernel_binder_event *b = &event->u.binder;

        payload = nla_nest_start(skb, REKERNEL_A_BINDER);
        if (!payload)
            goto nla_fail;
        if (nla_put_u8(skb, REKERNEL_A_BINDER_TYPE, b->binder_type) ||
            nla_put_u8(skb, REKERNEL_A_BINDER_ONEWAY, b->oneway) ||
            nla_put_s32(skb, REKERNEL_A_BINDER_FROM_PID, b->from_pid) ||
            nla_put_u32(skb, REKERNEL_A_BINDER_FROM_UID, b->from_uid) ||
            nla_put_s32(skb, REKERNEL_A_BINDER_TARGET_PID, b->target_pid) ||
            nla_put_u32(skb, REKERNEL_A_BINDER_TARGET_UID, b->target_uid) ||
            nla_put_s32(skb, REKERNEL_A_BINDER_CODE, b->code) ||
            nla_put_string(skb, REKERNEL_A_BINDER_RPC_NAME, b->rpc_name))
            goto nla_fail;
        nla_nest_end(skb, payload);
        break;
    }
    case REKERNEL_EVT_SIGNAL: {
        struct rekernel_signal_event *s = &event->u.signal;

        payload = nla_nest_start(skb, REKERNEL_A_SIGNAL);
        if (!payload)
            goto nla_fail;
        if (nla_put_s32(skb, REKERNEL_A_SIGNAL_SIGNAL, s->signal) ||
            nla_put_s32(skb, REKERNEL_A_SIGNAL_KILLER_PID, s->killer_pid) ||
            nla_put_u32(skb, REKERNEL_A_SIGNAL_KILLER_UID, s->killer_uid) ||
            nla_put_s32(skb, REKERNEL_A_SIGNAL_DST_PID, s->dst_pid) ||
            nla_put_u32(skb, REKERNEL_A_SIGNAL_DST_UID, s->dst_uid))
            goto nla_fail;
        nla_nest_end(skb, payload);
        break;
    }
    case REKERNEL_EVT_NETWORK: {
        struct rekernel_network_event *n = &event->u.network;

        payload = nla_nest_start(skb, REKERNEL_A_NETWORK);
        if (!payload)
            goto nla_fail;
        if (nla_put_u8(skb, REKERNEL_A_NETWORK_PROTO, n->proto) ||
            nla_put_u32(skb, REKERNEL_A_NETWORK_TARGET_UID, n->target_uid) ||
            nla_put_s32(skb, REKERNEL_A_NETWORK_DATA_LEN, n->data_len))
            goto nla_fail;
        nla_nest_end(skb, payload);
        break;
    }
    ...
    rc = genlmsg_multicast(&rekernel_genl_family, skb, 0, 0, GFP_ATOMIC);
    if (rc && rc != -ESRCH) {
